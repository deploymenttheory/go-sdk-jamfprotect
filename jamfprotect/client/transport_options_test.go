...
func TestWithGlobalHeader_NilMap(t *testing.T) {
	settings := &TransportSettings{GlobalHeaders: nil}
	err := WithGlobalHeader("X-Custom", "value")(settings)
	require.NoError(t, err)
	assert.NotNil(t, settings.GlobalHeaders)
	assert.Equal(t, "value", settings.GlobalHeaders["X-Custom"])
}
